...
def _run_sqlite_migrations() -> None:
    with engine.begin() as conn:
        ...
        conn.execute(text("CREATE TABLE IF NOT EXISTS trial_attempts (id INTEGER PRIMARY KEY, ip_address TEXT NULL, contact_email TEXT NULL, phone TEXT NULL, company_name TEXT NULL, created_at DATETIME NOT NULL, success INTEGER NOT NULL DEFAULT 0)"))
        ...
        conn.execute(text("CREATE INDEX IF NOT EXISTS idx_trial_attempts_ip ON trial_attempts(ip_address)"))
        conn.execute(text("CREATE INDEX IF NOT EXISTS idx_trial_attempts_email ON trial_attempts(contact_email)"))
        conn.execute(text("CREATE INDEX IF NOT EXISTS idx_trial_attempts_phone ON trial_attempts(phone)"))
